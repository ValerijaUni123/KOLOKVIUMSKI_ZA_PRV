...
void printResults() {
    ...
    LogProcessor<T> firstLogProcessor = x -> x.stream().filter(i -> i.getType().equals("INFO"))
            .collect(Collectors.toCollection(ArrayList::new));

    LogProcessor<T> secondLogProcessor = x -> x.stream().filter(i -> i.getMessage().length() < 100)
            .collect(Collectors.toCollection(ArrayList::new));

    LogProcessor<T> thirdLogProcessor = x -> x.stream().sorted()
            .collect(Collectors.toCollection(ArrayList::new));
